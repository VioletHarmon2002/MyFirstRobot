...
2. **`-it`**  
   ...
   - **`-i` (interactive mode)**: Keeps the standard input (stdin) open, allowing you to interact with the container.
   - **`-t` (pseudo-TTY)**: Allocates a terminal for the session, making it behave like a typical command-line interface.
